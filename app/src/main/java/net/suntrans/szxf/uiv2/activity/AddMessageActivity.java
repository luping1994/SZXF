...
public class AddMessageActivity extends BasedActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_CHOOSE && resultCode == RESULT_OK) {
            ...
            LogUtil.i("Matisse", "mSelected: " + mSelected);
            LogUtil.i("Matisse", "paths: " + paths);
            adapter.notifyDataSetChanged();
...
//        upload();
    ...
    private void storeNotice(String title, String contents, String vtype, String images) {
        ...
        addSubscription(api.storeNotice(map), new BaseSubscriber<RespondBody>(this) {
            @Override
            public void onNext(RespondBody body) {
        ...
        mCompositeSubscription.add(Observable.from(paths)
                .map(new Func1<String, File>() {
                    ...
                    public File call(String path) {
                        //压缩图片并且返回压缩后的图片文件
                        File cacheDir = App.getApplication().getCacheDir();
