...
import net.suntrans.szxf.databinding.FragmentChoosePicBinding;
...
public class PicChooseFragment extends DialogFragment {
    ...
    private void getData() {
        mCompositeSubscription.add(api.getSceneDefaultImg()
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new BaseSubscriber<RespondBody<SceneImage>>(getContext()) {
                    @Override
                    public void onNext(RespondBody<SceneImage> channelEntityResultBody) {
                        datas.clear();
